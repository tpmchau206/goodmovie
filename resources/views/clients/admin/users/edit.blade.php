...
<div class="tab-pane fade show active overflow-x-hidden overflow-y-auto" style="height: 90vh" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab" tabindex="0">
    ...
    <form action="{{ route('admin.users.post-edit') }}" class="d-flex flex-column" id="form" method="post" enctype="multipart/form-data">
        {{-- @method('POST') --}}
        <div class="p-1 col-6">
            <label for="">Tên người dùng</label>
            <input type="text" class="form-control" name="username" placeholder="Tên người dùng" value="{{$user[0]->username}}">
            @error('username')
        ...
        <div class="p-1 col-6" style="">
            <div class="d-flex flex-column">
                ...
                <input type="text" class="form-control" name="email" placeholder="Email" value="{{$user[0]->email}}">
            </div>
            @error('email')
            <span style="color: red">{{$message}}</span>
            @enderror
        ...
        <div class="p-1 col-6">
            ...
            <input type="text" class="form-control" name="fullname" placeholder="Họ và tên" value="{{$user[0]->fullname}}">
            @error('fullname')
            <span style="color: red">{{$message}}</span>
            @enderror
        </div>
        ...
        <div class="p-1 col-6">
            ...
            <input type="date" class="form-control" name="dateofbirth" placeholder="Ngày sinh" value="{{$user[0]->dateofbirth}}">
            @error('dateofbirth')
            <span style="color: red">{{$message}}</span>
            @enderror
        </div>
        <div class="d-flex col-6">
            <div class="p-1 col-6">
                ...
                <select class="form-control" name="group_id">
                    ...
                    <option value="{{$item->id}}" {{$user[0]->group_id==$item->id?'selected':false}}>{{$item->name}}</option>
                    @endforeach
                    {{-- <option value="1" {{$user[0]->group_id==1?'selected':''}}>Admin</option>  --}}
            ...
            <div class="p-1 col-6">
                ...
                <select class="form-control" name="status" id="">
                    <option value="0" {{$user[0]->status==0?'selected':''}}>Chưa kích hoạt</option>
                    <option value="1" {{$user[0]->status==1?'selected':''}}>Kích hoạt</option>
                </select>
                @error('status')
                <span style="color: red">{{$message}}</span>
                @enderror
